...
defmodule Bulkhead.Mission.Raid do
  ...
  def render(%{status: :loading} = state) do
    # Считаем прогресс загрузки для статус-бара
    total_steps = 3
    current_step = total_steps - state.loading_ticks
    progress = progress_bar(current_step, total_steps)

    %{
      title: "🛰️ ПОДГОТОВКА К АБОРДАЖУ: " <> mission_name(),
      # Синий цвет на этапе брифинга
      color: 0x3498DB,
      description: """
      **Внимание, пилоты!** Система синхронизирована. Изучите тактическую задачу, пока завершается сканирование сектора.

      ### 📋 ПЛАН ОПЕРАЦИИ
      1️⃣ **СБЛИЖЕНИЕ**: Сбиваем щиты конвоя. Появятся кнопки **тактики** (уворот, форсаж, залп).
      2️⃣ **ВЗЛОМ**: Один из вас должен стать **Взломщиком** (ускоряет прогресс), остальные — **Прикрытием** (снижают урон).
      3️⃣ **ШТУРМ**: Прорываемся внутрь за лутом. Можно рискнуть и взорвать реактор для быстрой победы!

      ### ⚠️ ВАЖНЫЕ ПРАВИЛА
      • **Смерть одного = Провал всех.** Следите за HP союзников!
      • **Кооперация:** Кнопки действий влияют на всю группу. Не жмите всё подряд!
      • **Роли:** Выбор роли на фазе 2 определит ваш бонус до конца рейда.
      ...
      **Статус готовности:**
      ...
      fields: [
        %{
          name: "👥 Состав группы",
          value: Enum.map_join(state.ships, "\n", fn s -> "• 🛸 Корабль ##{s.ship_id}" end),
          inline: false
        }
      ],
      footer: %{text: "Ожидание прыжковых двигателей... #{state.loading_ticks} сек."}
    }
  end
  ...
  def render(state) do
    ...
    phase_hint =
      case state.phase do
        :approach -> "🎯 *Совет: Сбивайте щиты! Используйте 'Уклонение', если HP низкое.*"
        :breach -> "🔧 *Совет: Распределите роли! Без Взломщика шлюз поддается медленно.*"
        :assault -> "📦 *Совет: Набивайте трюмы или бейте в реактор, если врагов слишком много!*"
      end
